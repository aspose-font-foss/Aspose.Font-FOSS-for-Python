...
def test_send_task_completion_report_requires_configured_token() -> None:
    try:
        reporting.send_task_completion_report(
            {"task_id": "FONT-123", "token_usage": 321},
            endpoint="https://example.invalid/exec",
        )
    except ValueError as exc:
        assert "ASPOSE_FONT_TOKEN_REPORT_TOKEN" in str(exc)
    else:
        raise AssertionError("send_task_completion_report should reject missing token")


def test_send_task_completion_report_requires_configured_endpoint() -> None:
    try:
        reporting.send_task_completion_report(
            {"task_id": "FONT-123", "token_usage": 321},
            token="secret",
        ...
        assert "ASPOSE_FONT_TOKEN_REPORT_ENDPOINT" in str(exc)
    else:
        raise AssertionError("send_task_completion_report should reject missing endpoint")
