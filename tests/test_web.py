...
def test_web_optimizer_builds_handoff_manifest(roboto: TtfFont):
    package = WebFontOptimizer.build(
        roboto,
        source_path=Path("testdata/Roboto-VariableFont_wdth,wght.ttf"),
        presets=("latin",),
        text="Aspose Web",
        include_woff=False,
    )

    assert package.bundle.manifest["export_mode"] == "static-subset-from-variable-default"
    assert package.manifest["kind"] == "web_optimizer_package"
    assert package.manifest["local_processing"] is True
    assert package.size_summary["source_bytes"] is not None
    assert package.size_summary["total_web_font_bytes"] > 0
    assert package.size_summary["total_package_bytes"] > package.size_summary["total_web_font_bytes"]
    assert package.readiness["status"] == "review"
    assert package.readiness["score"] == 65
    assert "subset-missing-codepoints" in package.readiness["reasons"]
    assert "variable-subset-static-fallback" in package.readiness["reasons"]
    assert any(note["code"] == "subset-missing-codepoints" for note in package.notes)
    assert any(note["code"] == "variable-subset-static-fallback" for note in package.notes)
    assert package.manifest["bundle"]["subset"]["coverage"]["covered_count"] >= 1


def test_web_optimizer_package_writes_bundle_and_optimizer_manifest(
    roboto: TtfFont,
    tmp_path: Path,
):
    package = WebFontOptimizer.build(
    ...
    assert tmp_path / "roboto-regular.woff2" in written
    assert (tmp_path / "roboto-regular.css").exists()
    assert (tmp_path / "roboto-regular.html").exists()
    ...
    assert manifest["readiness"]["score"] == 95
    assert manifest["size_summary"]["font_assets"][0]["filename"] == "roboto-regular.woff2"
...
def test_web_bundle_manifest_includes_stat_policy_recommendation(roboto: TtfFont):
    ...
    assert bundle.manifest["supported_stat_policies"] == ["drop", "static"]
    ...
    assert bundle.manifest["stat_policy_next_action"] == "keep-static"
...
def test_web_bundle_variable_export_surfaces_axis_metadata(roboto: TtfFont):
    ...
    assert bundle.manifest["supported_stat_policies"] == []
    assert bundle.manifest["stat_policy_recommendation"] is None
    assert bundle.manifest["stat_policy_recommendation_reasons"] == []
    assert bundle.manifest["stat_policy_next_action"] is None
    assert bundle.manifest["stat_policy_override_suggestion"] is None
    assert bundle.manifest["stat_policy_override_suggestion_reasons"] == []
...
def test_smart_instancer_builds_axis_grid_web_family_package(roboto: TtfFont):
    ...
    assert package.manifest["bundles"][1]["supported_stat_policies"] == ["drop", "static"]
    ...
    assert package.manifest["bundles"][1]["stat_policy_next_action"] == "manual-review"
