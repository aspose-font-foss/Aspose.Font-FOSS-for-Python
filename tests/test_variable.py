...
from aspose_font.ttf.tables.stat import (
    ...
    describe_stat_axis_value_flags,
    extract_stat_axis_value_flags,
    extract_stat_name_ids,
    summarize_stat_axis_value_flags,
)
...
def _synthetic_stat_table() -> bytes:
    ...
    axis_value_records = (
        ...
        + u16(1) + u16(1) + u16(3) + u16(60003) + u32(75 << 16)
    )
    return (
        u16(1)
        + u16(1)
...
def test_roboto_exports_variable_presentation_snapshot(roboto: TtfFont) -> None:
    ...
    assert snapshot["schema_name"] == "aspose-font.variable-presentation"
...
def test_variable_presentation_schema_contract_stable_fields(roboto: TtfFont) -> None:
    snapshot = roboto.variable_presentation(preferred_languages=("fr-CA", "en"))

    required_font_types = {
        "schema_name": str,
        "schema_version": str,
        "schema_revision": int,
        "font_name": (str, type(None)),
        "font_family": (str, type(None)),
        "font_style": (str, type(None)),
        "is_variable": bool,
    ...
    for field, expected_type in required_font_types.items():
        assert field in snapshot
        assert isinstance(snapshot[field], expected_type)

    axis_group = snapshot["axis_groups"][0]
    for field, expected_type in {
        ...
        assert field in axis_group
        assert isinstance(axis_group[field], expected_type)

    axis = next(item for item in snapshot["axes"] if item["tag"] == "wght")
    for field, expected_type in {
        "tag": str,
        "name_id": int,
        "label": str,
        ...
        "localization": dict,
        "localization_coverage": dict,
        "available_languages": list,
        "localized_labels": list,
        "requested_language_hints": list,
        "language_profiles": list,
        "min_value": float,
        "default_value": float,
        "max_value": float,
        ...
        "formatted_max": str,
        "css_axis_tag": str,
        "css_default_setting": str,
        "ui_control": dict,
        "range_summary": str,
        "default_description": str,
        "default_ratio": float,
        "normalized_default": float,
        "is_hidden": bool,
        ...
        "presets": list,
        "default_preset": (dict, type(None)),
        "suggested_values": list,
    }.items():
        assert field in axis
        assert isinstance(axis[field], expected_type)

    axis_profile = axis["language_profiles"][0]
    assert set(axis_profile) >= {
        "requested_language",
        "display_label",
        "resolved_language",
        "fallback_reason",
        "has_requested_language_label",
    }

    instance = next(item for item in snapshot["named_instances"] if item["label"] == "Bold")
    for field, expected_type in {
        "name_id": int,
        "postscript_name_id": (int, type(None)),
        "label": str,
        "postscript_name": (str, type(None)),
        "localization": dict,
        "localization_coverage": dict,
        "available_languages": list,
        "localized_labels": list,
        ...
        "language_profiles": list,
        "coordinates": dict,
        "formatted_coordinates": list,
        "tagged_coordinates": list,
        "css_variation_settings": str,
        "css_variation_setting_entries": list,
    }.items():
        assert field in instance
        assert isinstance(instance[field], expected_type)
...
def test_preview_naming_policy_reports_stat_diagnostics_without_source_stat(roboto: TtfFont) -> None:
    ...
    assert preview.stat_diagnostics.supported_stat_policies == ("drop", "static")
    assert preview.stat_diagnostics.source_has_stat is False
    assert preview.stat_diagnostics.static_export_action == "none"
    assert preview.stat_diagnostics.stat_policy_recommendation == "no-action"
    assert preview.stat_diagnostics.stat_policy_recommendation_reasons == ("source-stat-absent",)
    assert preview.stat_diagnostics.stat_policy_next_action == "none"
    assert preview.stat_diagnostics.stat_policy_override_suggestion is None
    assert preview.stat_diagnostics.stat_policy_override_suggestion_reasons == (
        "no-override-needed",
    )
    ...
    assert preview.stat_diagnostics.source_stat_axis_value_flags == ()
    assert preview.stat_diagnostics.source_stat_axis_value_flag_meanings == ()
    assert preview.stat_diagnostics.source_stat_axis_value_flag_summary == {
        "total": 0,
        "nonzero": 0,
        "older_sibling": 0,
        "elidable_axis_value_name": 0,
        "unknown": 0,
    }
    assert preview.stat_diagnostics.warnings == ()
    assert preview.to_dict()["stat_diagnostics"]["source_stat_axis_value_flags"] == []
    assert preview.to_dict()["stat_diagnostics"]["source_stat_axis_value_flag_meanings"] == []
    assert preview.to_dict()["stat_diagnostics"]["source_stat_axis_value_flag_summary"] == {
        "total": 0,
        "nonzero": 0,
        "older_sibling": 0,
        "elidable_axis_value_name": 0,
    ...
    assert preview.to_dict()["stat_diagnostics"]["stat_policy_next_action"] == "none"
    assert preview.to_dict()["stat_diagnostics"]["supported_stat_policies"] == [
        "drop",
        "static",
    ]
    assert preview.to_dict()["stat_diagnostics"]["stat_policy_override_suggestion"] is None
    assert preview.to_dict()["stat_diagnostics"]["stat_policy_override_suggestion_reasons"] == [
        "no-override-needed"
    ]
...
def test_preview_naming_policy_tolerates_malformed_source_stat(roboto: TtfFont) -> None:
    ...
    assert preview.stat_diagnostics.source_stat_axis_value_flags == ()
    assert preview.stat_diagnostics.source_stat_axis_value_flag_meanings == ()
    assert preview.stat_diagnostics.source_stat_axis_value_flag_summary["total"] == 0
    assert preview.stat_diagnostics.static_export_action == "drop-source-stat"
    assert preview.stat_diagnostics.stat_policy_recommendation == "drop-with-diagnostics"
    assert preview.stat_diagnostics.stat_policy_recommendation_reasons == (
        "source-stat-dropped-by-default",
        "source-stat-present-without-parseable-details",
    )
    assert preview.stat_diagnostics.stat_policy_next_action == "keep-drop"
    assert preview.stat_diagnostics.stat_policy_override_suggestion == "drop"
    assert preview.stat_diagnostics.stat_policy_override_suggestion_reasons == (
        "drop-policy-currently-safe",
    )
...
def test_preview_naming_policy_reports_source_stat_name_id_coverage(roboto: TtfFont) -> None:
    ...
    assert preview.stat_diagnostics.source_stat_axis_value_flags == (0, 3)
    assert preview.stat_diagnostics.source_stat_axis_value_flag_meanings == (
        (0, ()),
        (3, ("older-sibling", "elidable-axis-value-name")),
    )
    assert preview.stat_diagnostics.source_stat_axis_value_flag_summary == {
        "total": 2,
        "nonzero": 1,
    ...
    assert preview.stat_diagnostics.stat_policy_recommendation == "review-before-drop"
    assert preview.stat_diagnostics.stat_policy_recommendation_reasons == (
        "source-stat-dropped-by-default",
        "source-stat-name-ids-uncovered",
    )
    assert preview.stat_diagnostics.stat_policy_next_action == "manual-review"
    assert preview.stat_diagnostics.stat_policy_override_suggestion is None
    assert preview.stat_diagnostics.stat_policy_override_suggestion_reasons == (
    ...
    assert payload["stat_diagnostics"]["source_stat_axis_value_flags"] == [0, 3]
    assert payload["stat_diagnostics"]["source_stat_axis_value_flag_meanings"] == [
        {"flags": 0, "meanings": []},
        {"flags": 3, "meanings": ["older-sibling", "elidable-axis-value-name"]},
    ]
    assert payload["stat_diagnostics"]["source_stat_axis_value_flag_summary"] == {
        "total": 2,
        "nonzero": 1,
        "older_sibling": 1,
        "elidable_axis_value_name": 1,
...
def test_describe_stat_axis_value_flags_reports_known_zero_and_unknown_bits() -> None:
    assert describe_stat_axis_value_flags(0) == ()
    assert describe_stat_axis_value_flags(0x0001) == ("older-sibling",)
    assert describe_stat_axis_value_flags(0x0002) == ("elidable-axis-value-name",)
    assert describe_stat_axis_value_flags(0x8003) == (
        "older-sibling",
...
def test_summarize_stat_axis_value_flags_reports_usage_counts() -> None:
    assert summarize_stat_axis_value_flags((0, 1, 2, 3, 0x8000, 0x8003)) == {
        "total": 6,
        "nonzero": 5,
        "older_sibling": 3,
        "elidable_axis_value_name": 3,
        "unknown": 2,
    }


def test_preview_naming_policy_recommends_manual_review_for_unknown_source_flags(
    roboto: TtfFont,
) -> None:
    original_stat = roboto.ttf_tables._raw.get("STAT")
    roboto.ttf_tables._raw["STAT"] = build_static_stat_table(
        roboto.axes,
        {"wght": 700.0, "wdth": 75.0},
        axis_value_flags=0x8000,
    )
    try:
        preview = roboto.preview_naming_policy({"wght": 700.0, "wdth": 75.0})
    finally:
        if original_stat is None:
            roboto.ttf_tables._raw.pop("STAT", None)
        else:
    ...
    assert preview.stat_diagnostics is not None
    assert preview.stat_diagnostics.source_stat_axis_value_flag_summary["unknown"] == 1
    assert preview.stat_diagnostics.stat_policy_recommendation == "review-before-drop"
    assert preview.stat_diagnostics.stat_policy_recommendation_reasons == (
        "source-stat-dropped-by-default",
        "source-axis-value-flags-unknown",
    )
    assert preview.stat_diagnostics.stat_policy_override_suggestion is None
    assert preview.stat_diagnostics.stat_policy_override_suggestion_reasons == (
        "manual-review-required",
...
def test_stat_policy_recommendation_considers_static_for_nonzero_covered_source_flags() -> None:
    recommendation, reasons = TtfInstancer._stat_policy_recommendation(
        stat_policy="drop",
        source_has_stat=True,
        source_stat_name_ids=(2, 17),
        uncovered_source_stat_name_ids=(),
        source_stat_axis_value_flag_summary={
            "total": 1,
            "nonzero": 1,
            "older_sibling": 1,
            "elidable_axis_value_name": 1,
    ...
    assert recommendation == "consider-static"
    assert reasons == (
        "source-stat-dropped-by-default",
        "source-axis-value-flags-nonzero",
    )
    suggestion, suggestion_reasons = TtfInstancer._stat_policy_override_suggestion(recommendation)
    assert suggestion == "static"
    assert suggestion_reasons == ("try-static-policy",)
    assert TtfInstancer._stat_policy_next_action(recommendation) == "try-static"


def test_stat_policy_next_action_maps_known_recommendations() -> None:
    assert TtfInstancer._stat_policy_next_action("no-action") == "none"
    assert TtfInstancer._stat_policy_next_action("drop-with-diagnostics") == "keep-drop"
    assert TtfInstancer._stat_policy_next_action("consider-static") == "try-static"
    assert TtfInstancer._stat_policy_next_action("review-before-drop") == "manual-review"
    assert TtfInstancer._stat_policy_next_action("use-requested-static") == "keep-static"
    assert TtfInstancer._stat_policy_next_action("future-policy") == "manual-review"
...
def test_build_static_stat_table_references_axis_and_generated_name_ids(roboto: TtfFont) -> None:
    ...
    assert extract_stat_axis_value_flags(data) == (0,)


def test_build_static_stat_table_can_emit_axis_value_flags(roboto: TtfFont) -> None:
    data = build_static_stat_table(
        roboto.axes,
        {"wght": 700.0, "wdth": 75.0},
        axis_value_flags=0x0003,
    )

    assert extract_stat_axis_value_flags(data) == (0x0003,)
...
def test_preview_naming_policy_reports_static_stat_generation(roboto: TtfFont) -> None:
    ...
    assert preview.stat_diagnostics.stat_policy_recommendation == "use-requested-static"
    assert preview.stat_diagnostics.stat_policy_recommendation_reasons == (
        "static-policy-requested",
        "generated-static-stat-synthesized",
    )
    assert preview.stat_diagnostics.stat_policy_next_action == "keep-static"
    assert preview.stat_diagnostics.stat_policy_override_suggestion is None
    assert preview.stat_diagnostics.stat_policy_override_suggestion_reasons == (
    ...
    assert preview.stat_diagnostics.generated_stat_axis_value_flags == 0
    assert preview.stat_diagnostics.generated_stat_axis_value_flag_meanings == ()
    assert preview.stat_diagnostics.generated_stat_axis_value_flag_summary == {
        "total": 1,
        "nonzero": 0,
        "older_sibling": 0,
        "elidable_axis_value_name": 0,
...
def test_preview_naming_policy_reports_ribbi_safe_static_stat_linked_flags(roboto: TtfFont) -> None:
    preview = roboto.preview_naming_policy(
        {"wght": 700.0, "wdth": 75.0},
        naming_strategy="ribbi-safe",
        stat_policy="static",
    ...
    assert preview.stat_diagnostics is not None
    assert preview.stat_diagnostics.generated_stat_axis_value_flags == 0x0003
    assert preview.stat_diagnostics.stat_policy_recommendation == "use-requested-static"
    assert preview.stat_diagnostics.stat_policy_recommendation_reasons == (
        "static-policy-requested",
        "generated-static-stat-synthesized",
    ...
    assert preview.to_dict()["stat_diagnostics"]["generated_stat_axis_value_flag_meanings"] == [
        "older-sibling",
        "elidable-axis-value-name",
    ]
    assert preview.to_dict()["stat_diagnostics"]["generated_stat_axis_value_flag_summary"] == {
        "total": 1,
        "nonzero": 1,
        "older_sibling": 1,
        "elidable_axis_value_name": 1,
...
def test_instantiate_static_stat_policy_emits_stat_table(roboto: TtfFont) -> None:
    ...
    assert extract_stat_axis_value_flags(instantiated.ttf_tables._raw["STAT"]) == (0,)


def test_instantiate_ribbi_safe_static_stat_policy_emits_linked_flags(roboto: TtfFont) -> None:
    instantiated = roboto.instantiate(
        {"wght": 700.0, "wdth": 75.0},
        naming_strategy="ribbi-safe",
        stat_policy="static",
    ...
    assert "STAT" in instantiated.ttf_tables._raw
    assert extract_stat_axis_value_flags(instantiated.ttf_tables._raw["STAT"]) == (0x0003,)
