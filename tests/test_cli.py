...
def test_web_optimize_writes_optimizer_package(tmp_path: Path):
    out_dir = tmp_path / "optimized-web"
    result = run(
        "web-optimize",
        ROBOTO,
        str(out_dir),
    ...
    assert (out_dir / "roboto-instance-regular.css").exists()
    assert (out_dir / "roboto-instance-regular.html").exists()
    assert (out_dir / "web-manifest.json").exists()
    manifest_path = out_dir / "optimizer-manifest.json"
    assert manifest_path.exists()
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    assert manifest["kind"] == "web_optimizer_package"
    assert manifest["size_summary"]["source_bytes"] > 0
    assert manifest["size_summary"]["total_web_font_bytes"] > 0
    assert manifest["readiness"]["status"] == "review"
    assert "Optimizer status: review" in result.stdout
    assert "Optimizer score: 65" in result.stdout
    assert "Note variable-subset-static-fallback:" in result.stdout
...
def test_web_build_records_static_stat_policy(tmp_path: Path):
    out_dir = tmp_path / "variable-web-static-stat"
    result = run(
        "web-build",
        ROBOTO,
        str(out_dir),
    ...
    assert result.returncode == 0
    manifest = json.loads((out_dir / "web-manifest.json").read_text(encoding="utf-8"))
    assert manifest["requested_stat_policy"] == "static"
    assert manifest["supported_stat_policies"] == ["drop", "static"]
    assert manifest["export_mode"] == "static-instance"
    assert manifest["stat_policy_recommendation"] == "use-requested-static"
    assert manifest["stat_policy_recommendation_reasons"] == [
        "static-policy-requested",
        "generated-static-stat-synthesized",
    ]
    assert manifest["stat_policy_next_action"] == "keep-static"
    assert manifest["stat_policy_override_suggestion"] is None
    assert manifest["stat_policy_override_suggestion_reasons"] == [
...
def test_var_info_accepts_preferred_language_order():
    ...
    assert "Label fallback: requested=fr-ca, en; selected=en; reason=exact" not in result.stdout
...
def test_var_info_prints_localized_label_inventory_when_available(tmp_path: Path):
    ...
    assert result.returncode == 0
    assert "Label fallback: requested=pt-pt, fr-ca; selected=pt-br; reason=locale-variant" in result.stdout
...
def test_var_info_prints_partial_localization_coverage(tmp_path: Path):
    font = FontLoader.open(ROBOTO)
    name_table = font.ttf_tables.name
    assert name_table is not None
    weight_axis = font.axes[0]
    name_table.records.append(NameRecord(3, 1, 0x040C, weight_axis.name_id, "Poids"))
    localized = tmp_path / "localized-partial.ttf"
    localized.write_bytes(font.to_bytes())

    result = run("var-info", str(localized), "--language", "fr-FR", "--language", "ja-JP")

    assert result.returncode == 0
    assert (
        "Localization coverage: partial "
        "(requested=fr-fr, ja-jp; available=en, fr; missing=ja-jp)"
    ) in result.stdout


def test_var_info_prints_first_available_localization_fallback(tmp_path: Path):
    font = FontLoader.open(ROBOTO)
    name_table = font.ttf_tables.name
    assert name_table is not None
    weight_axis = font.axes[0]
    name_table.remove_name_ids([weight_axis.name_id])
    name_table.records.append(NameRecord(3, 1, 0x0407, weight_axis.name_id, "Gewicht"))
    localized = tmp_path / "localized-first-available.ttf"
    localized.write_bytes(font.to_bytes())

    result = run("var-info", str(localized), "--language", "ja-JP")

    assert result.returncode == 0
    assert "Label fallback: requested=ja-jp; selected=de; reason=first-available" in result.stdout
    assert "Localization coverage: missing (requested=ja-jp; available=de; missing=ja-jp)" in result.stdout


def test_var_info_prints_english_only_localization_coverage():
    result = run("var-info", ROBOTO, "--language", "fr-FR", "--language", "ja-JP")

    assert result.returncode == 0
    assert (
        "Localization coverage: english-only "
        "(requested=fr-fr, ja-jp; available=en; missing=fr-fr, ja-jp)"
    ) in result.stdout
...
def test_var_info_writes_presentation_json(tmp_path: Path):
    ...
    assert payload["language_profiles"][0]["requested_language"] == "en"
    assert payload["language_profiles"][0]["axes"]["wght"]["display_label"] == "Weight"
    assert payload["language_profiles"][0]["axis_groups"]["style"]["display_label"] == "Style"
    ...
    assert weight["localization_coverage"] == {
        "status": "complete",
        "requested_languages": ["en"],
    ...
    assert weight["ui_control"]["label"] == "Weight"
    ...
    bold_instance = next(instance for instance in payload["named_instances"] if instance["label"] == "Bold")
...
def test_var_batch_accepts_name_overrides(tmp_path: Path):
    out_dir = tmp_path / "batch-overrides"
    result = run(
        "var-batch",
        ROBOTO,
        str(out_dir),
    ...
    assert result.returncode == 0
    out = out_dir / "acme-sans-menu-condensed-bold.ttf"
    assert out.exists()
    loaded = FontLoader.open(str(out))
...
def test_var_naming_preview_prints_effective_name_ids(tmp_path: Path):
    json_out = tmp_path / "naming-preview.json"
    result = run(
        "var-naming-preview",
        ROBOTO,
        "--instance-name",
    ...
    assert result.returncode == 0
    assert "Naming strategy: ribbi-safe" in result.stdout
    assert "Source instance: Condensed Bold" in result.stdout
    assert "  1: Acme Sans Menu" in result.stdout
    assert "  2: Bold" in result.stdout
    assert "  6: AcmeSansMenu-CondensedDisplayBold" in result.stdout
    assert "STAT diagnostics:" in result.stdout
    assert "Supported STAT policies: drop,static" in result.stdout
    assert "Source STAT: yes" in result.stdout
    assert "Typographic IDs emitted: family=16,21,25; style=17,22" in result.stdout
    assert "STAT policy recommendation:" in result.stdout
    assert "STAT policy recommendation reasons:" in result.stdout
    assert "STAT policy next action: manual-review" in result.stdout
    assert "STAT policy override suggestion: -" in result.stdout
    assert "STAT policy override suggestion reasons: manual-review-required" in result.stdout
    assert "Source STAT name IDs:" in result.stdout
    assert "Covered source STAT name IDs:" in result.stdout
    assert "Uncovered source STAT name IDs:" in result.stdout
    assert "Source STAT labels:" in result.stdout
    assert "Source STAT AxisValue flags:" in result.stdout
    assert "Source STAT AxisValue flag meanings:" in result.stdout
    assert "Source STAT AxisValue flag summary:" in result.stdout
    assert "elidable-axis-value-name" in result.stdout
    assert " (covered)" in result.stdout
    assert " (uncovered)" in result.stdout
    assert "Platform diagnostics:" in result.stdout
    assert "Windows legacy menu safe: True" in result.stdout
    assert "PostScript name safe: True" in result.stdout
    payload = json.loads(json_out.read_text(encoding="utf-8"))
    assert payload["source_instance_name"] == "Condensed Bold"
    assert payload["name_ids"]["16"] == "Acme Sans Pro"
    assert payload["name_ids"]["17"] == "Condensed Display Bold"
    assert payload["stat_diagnostics"]["source_has_stat"] is True
    assert payload["stat_diagnostics"]["supported_stat_policies"] == ["drop", "static"]
    assert payload["stat_diagnostics"]["static_export_action"] == "drop-source-stat"
    assert payload["stat_diagnostics"]["stat_policy_recommendation"] == "review-before-drop"
    assert payload["stat_diagnostics"]["stat_policy_recommendation_reasons"]
    assert payload["stat_diagnostics"]["stat_policy_next_action"] == "manual-review"
    assert payload["stat_diagnostics"]["stat_policy_override_suggestion"] is None
    assert payload["stat_diagnostics"]["stat_policy_override_suggestion_reasons"] == [
        "manual-review-required"
    ]
    assert payload["stat_diagnostics"]["source_stat_name_ids"]
    assert "covered_source_stat_name_ids" in payload["stat_diagnostics"]
    assert "uncovered_source_stat_name_ids" in payload["stat_diagnostics"]
    assert payload["stat_diagnostics"]["source_stat_name_labels"]
    assert payload["stat_diagnostics"]["source_stat_axis_value_flags"]
    assert payload["stat_diagnostics"]["source_stat_axis_value_flag_meanings"]
    assert payload["stat_diagnostics"]["source_stat_axis_value_flag_summary"]["total"]
    assert all(
        {"name_id", "label", "covered"} <= set(item)
        for item in payload["stat_diagnostics"]["source_stat_name_labels"]
    )
    assert payload["stat_diagnostics"]["legacy_typographic_family_diverges"] is True
    assert payload["stat_diagnostics"]["legacy_typographic_style_diverges"] is True
    assert payload["platform_diagnostics"]["windows_legacy_menu_safe"] is True
    assert payload["platform_diagnostics"]["postscript_name_safe"] is True


def test_var_naming_preview_reports_static_stat_policy(tmp_path: Path):
    json_out = tmp_path / "naming-preview-static-stat.json"
    result = run(
        "var-naming-preview",
        ROBOTO,
        "--instance-name",
    ...
    assert result.returncode == 0
    assert "Policy: static" in result.stdout
    assert "Static export action: synthesize-static-stat" in result.stdout
    assert "Generated STAT axes:" in result.stdout
    assert "Generated STAT name IDs:" in result.stdout
    assert "Generated STAT AxisValue flags: 3" in result.stdout
    assert (
        "Generated STAT AxisValue flag meanings: older-sibling,elidable-axis-value-name"
        in result.stdout
    )
    assert "Generated STAT AxisValue flag summary:" in result.stdout
    assert "STAT policy next action: keep-static" in result.stdout
    assert "STAT policy override suggestion: -" in result.stdout
    assert (
        "STAT policy override suggestion reasons: requested-static-policy-already-applied"
        in result.stdout
    )
    payload = json.loads(json_out.read_text(encoding="utf-8"))
    assert payload["stat_diagnostics"]["stat_policy"] == "static"
    assert payload["stat_diagnostics"]["static_export_action"] == "synthesize-static-stat"
    assert payload["stat_diagnostics"]["stat_policy_recommendation"] == "use-requested-static"
    assert payload["stat_diagnostics"]["stat_policy_recommendation_reasons"] == [
        "static-policy-requested",
        "generated-static-stat-synthesized",
        "generated-axis-value-flags-nonzero",
    ]
    assert payload["stat_diagnostics"]["stat_policy_next_action"] == "keep-static"
    assert payload["stat_diagnostics"]["stat_policy_override_suggestion"] is None
    assert payload["stat_diagnostics"]["stat_policy_override_suggestion_reasons"] == [
        "requested-static-policy-already-applied"
    ]
    assert payload["stat_diagnostics"]["generated_stat_axis_tags"]
    assert payload["stat_diagnostics"]["generated_stat_name_ids"]
    assert payload["stat_diagnostics"]["generated_stat_axis_value_flags"] == 0x0003
    assert payload["stat_diagnostics"]["generated_stat_axis_value_flag_meanings"] == [
        "older-sibling",
        "elidable-axis-value-name",
    ]
    assert payload["stat_diagnostics"]["generated_stat_axis_value_flag_summary"] == {
        "total": 1,
        "nonzero": 1,
        "older_sibling": 1,
        "elidable_axis_value_name": 1,
        "unknown": 0,
    }


def test_var_instance_static_stat_policy_writes_stat_table(tmp_path: Path):
    out = tmp_path / "static-stat.ttf"
    result = run(
        "var-instance",
        ROBOTO,
        str(out),
    ...
    assert result.returncode == 0
    loaded = FontLoader.open(out)
    assert isinstance(loaded, TtfFont)
    assert loaded.is_variable is False
    assert "STAT" in loaded.ttf_tables._raw


def test_var_naming_preview_reports_collision_warnings():
    result = run(
        "var-naming-preview",
        ROBOTO,
        "--instance",
    ...
    assert "legacy-menu-collision" in result.stdout
    assert "postscript-collision" in result.stdout
    assert "windows-menu-collision" in result.stdout
    assert "postscript-platform-collision" in result.stdout
...
def test_web_batch_accepts_name_overrides(tmp_path: Path):
    out_dir = tmp_path / "web-batch-overrides"
    result = run(
        "web-batch",
        ROBOTO,
        str(out_dir),
    ...
    assert result.returncode == 0
    bundle_dir = out_dir / "acme-sans-menu-bold"
    assert (bundle_dir / "acme-sans-menu-bold.woff2").exists()
    manifest = json.loads((bundle_dir / "web-manifest.json").read_text(encoding="utf-8"))
    assert manifest["family"] == "Acme Sans Menu"
    assert manifest["style"] == "Bold"
    assert manifest["requested_legacy_family_name"] == "Acme Sans Menu"
    assert manifest["requested_typographic_family_name"] == "Acme Sans Pro"
    assert manifest["requested_legacy_style_name"] == "Bold"
    assert manifest["requested_typographic_style_name"] == "Condensed Bold"
