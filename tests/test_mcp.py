...
def test_web_build_generates_bundle(roboto_path: Path, tmp_path: Path) -> None:
    ...
    assert result["supported_stat_policies"] == ["drop", "static"]
    ...
    assert result["stat_policy_next_action"] == "manual-review"
    assert result["stat_policy_override_suggestion"] is None
    assert result["stat_policy_override_suggestion_reasons"] == [
...
def test_mcp_web_build_reports_live_empty_stat_recommendation(
    ...
    assert result["supported_stat_policies"] == []
    assert result["stat_policy_recommendation"] is None
    assert result["stat_policy_recommendation_reasons"] == []
    assert result["stat_policy_next_action"] is None
    assert result["stat_policy_override_suggestion"] is None
    assert result["stat_policy_override_suggestion_reasons"] == []
...
def test_web_family_package_generates_shared_package(roboto_path: Path, tmp_path: Path) -> None:
    ...
    assert result["bundles"][0]["supported_stat_policies"] == ["drop", "static"]
    ...
    assert result["bundles"][0]["stat_policy_next_action"] == "manual-review"
    assert result["bundles"][0]["stat_policy_override_suggestion"] is None
    assert result["bundles"][0]["stat_policy_override_suggestion_reasons"] == [
        "manual-review-required"
    ]
