...
def test_readme_keeps_core_public_sections() -> None:
    ...
    assert "## Real Outputs" in readme
    assert "## Development" in readme


def test_readme_reads_like_public_landing_page() -> None:
    readme = _readme()
    assert "Optimize fonts for the web" in readme
    assert "Review font quality" in readme
    assert "Create release-safe variable instances" in readme
    assert "Web Package Handoff" in readme
    assert "QA Report Package" in readme
