...
fonts, with zero required third-party runtime dependencies. Beyond format conversion, it adds
variable-font instancing, visual previews, web-font packaging, and QA/compatibility reporting —
all built on the same `Font` object returned by `FontLoader.open()` — and is usable from Python,
the bundled `aspose-font` command-line tool, or an optional MCP server for AI tooling.
...
flowchart TD
  subgraph StartingPoints["Starting Points"]
    direction LR
    i1["An existing TTF, CFF, TYPE1, WOFF, WOFF2, or EOT file (path, bytes, or stream)"]
  end
...
- Explore and instantiate variable fonts through `font.axes`, `font.named_instances`, and
  ...
  strategies (`menu-safe`, `ribbi-safe`) and optional STAT metadata.
- Render PNG and SVG previews and APNG animation sweeps with `FontPreviewBuilder`, and package
  browser-ready web-font bundles (WOFF2/WOFF, CSS, HTML, JSON manifests) with `WebFontBuilder`
  and `SmartInstancer.build_web_bundle()`.
- Generate Font QA Reports, variable-font compatibility checks, and glyph delta/interpolation
  diagnostics — including active `gvar` tuple-variation changes and contour/control-point
  movement — with `FontQaReporter`, `CompatibilityChecker`, and `DeltaInspector`.
...
Protocol](https://modelcontextprotocol.io/) server surface. Pin to `mcp<2.0` — the real, current
`mcp` 2.0.0 release on PyPI renamed `mcp.server.fastmcp` to `mcp.server.mcpserver`, and the bundled
server still imports the old path:
...
- `mcp` (`>=1.0`) — the optional `mcp` extra that enables the built-in MCP server surface; see
  [Installation](#installation) above for how to enable it and the version pin it currently needs.
...
`FontQaReporter.build_package(...)` produces a review folder with JSON, HTML, and a preview PNG:
...
`font.instantiate(...)` (a `SmartInstancer` convenience) resolves named coordinates into a static
...
### Variable Font Discovery: Axes and Named Instances
...
Equivalent CLI, including the plain naming-strategy options and the same custom legacy/
...
### Inspect, Convert, and Run QA From the CLI

General font inspection/conversion and QA-report/compatibility tooling are also available
directly from the command line, without writing any Python:
...
(`Dockerfile`). The test suite also regenerates the demo-site preview assets referenced under
[Additional Examples](#additional-examples) (`Makefile`'s `docs`/`serve-website` targets serve
`website/` locally at `http://127.0.0.1:8000`).
