...
_STAT_OLDER_SIBLING_FLAG = 0x0001
_STAT_ELIDABLE_AXIS_VALUE_NAME_FLAG = 0x0002
_STAT_LINKED_ELIDABLE_FLAGS = _STAT_OLDER_SIBLING_FLAG | _STAT_ELIDABLE_AXIS_VALUE_NAME_FLAG
...
class StatNamingDiagnostics:
    ...
    source_stat_axis_value_flag_summary: dict[str, int] = dataclasses.field(default_factory=dict)
    generated_stat_name_ids: tuple[int, ...] = ()
    generated_stat_axis_tags: tuple[str, ...] = ()
    generated_stat_axis_value_flags: int = 0
    generated_stat_axis_value_flag_meanings: tuple[str, ...] = ()
    generated_stat_axis_value_flag_summary: dict[str, int] = dataclasses.field(default_factory=dict)
    notes: tuple[str, ...] = ()
    warnings: tuple[str, ...] = ()
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "supported_stat_policies": list(self.supported_stat_policies),
            "source_has_stat": self.source_has_stat,
            "static_export_action": self.static_export_action,
            "stat_policy_recommendation": self.stat_policy_recommendation,
            "stat_policy_recommendation_reasons": list(
                self.stat_policy_recommendation_reasons
            ),
            "stat_policy_next_action": self.stat_policy_next_action,
            "stat_policy_override_suggestion": self.stat_policy_override_suggestion,
            "stat_policy_override_suggestion_reasons": list(
                self.stat_policy_override_suggestion_reasons
            ),
            ...
            "source_stat_axis_value_flags": list(self.source_stat_axis_value_flags),
            "source_stat_axis_value_flag_meanings": [
                {"flags": flags, "meanings": list(meanings)}
                for flags, meanings in self.source_stat_axis_value_flag_meanings
            ],
            "source_stat_axis_value_flag_summary": dict(self.source_stat_axis_value_flag_summary),
            "generated_stat_name_ids": list(self.generated_stat_name_ids),
            "generated_stat_axis_tags": list(self.generated_stat_axis_tags),
            "generated_stat_axis_value_flags": self.generated_stat_axis_value_flags,
            "generated_stat_axis_value_flag_meanings": list(
                self.generated_stat_axis_value_flag_meanings
            ),
            "generated_stat_axis_value_flag_summary": dict(
                self.generated_stat_axis_value_flag_summary
            ),
...
class TtfInstancer:
        ...
        *,
        ...
        if normalized_stat_policy == "static":
            ...
            raw_tables["STAT"] = build_static_stat_table(
                ...
                axis_value_flags=naming_preview.stat_diagnostics.generated_stat_axis_value_flags
                if naming_preview.stat_diagnostics is not None
                else 0,
            )
    ...
    def _stat_naming_diagnostics(
        ...
        *,
        ...
        source_stat_axis_value_flags = (
            extract_stat_axis_value_flags(stat_raw) if stat_raw is not None else ()
        )
        source_stat_axis_value_flag_meanings = tuple(
            (flags, describe_stat_axis_value_flags(flags))
            for flags in source_stat_axis_value_flags
        )
        source_stat_axis_value_flag_summary = summarize_stat_axis_value_flags(
            source_stat_axis_value_flags
        )
        ...
        generated_stat_axis_value_flags = (
            TtfInstancer._static_stat_axis_value_flags(
                stat_policy,
                naming_strategy,
                legacy_style_label,
            )
        ...
        generated_stat_axis_value_flag_meanings = describe_stat_axis_value_flags(
            generated_stat_axis_value_flags
        )
        generated_stat_axis_value_flag_summary = summarize_stat_axis_value_flags(
            (generated_stat_axis_value_flags,) if stat_policy == "static" else ()
        )
        ...
        if stat_policy == "static":
            ...
            if source_stat_axis_value_flags:
                notes.append(
                    "Source STAT AxisValue flags are "
                    f"{','.join(str(flag) for flag in source_stat_axis_value_flags)}."
                )
                decoded_source_flags = "; ".join(
                    f"{flags}={','.join(meanings) or '-'}"
                    for flags, meanings in source_stat_axis_value_flag_meanings
                )
                notes.append(f"Source STAT AxisValue flag meanings are {decoded_source_flags}.")
                notes.append(
                    "Source STAT AxisValue flag summary: "
                    f"total={source_stat_axis_value_flag_summary['total']}; "
                    f"nonzero={source_stat_axis_value_flag_summary['nonzero']}; "
                    f"older-sibling={source_stat_axis_value_flag_summary['older_sibling']}; "
                    "elidable-axis-value-name="
                    f"{source_stat_axis_value_flag_summary['elidable_axis_value_name']}; "
                    f"unknown={source_stat_axis_value_flag_summary['unknown']}."
                )
            warnings.append(
        ...
        stat_policy_recommendation, stat_policy_recommendation_reasons = (
            TtfInstancer._stat_policy_recommendation(
                stat_policy=stat_policy,
                ...
                source_stat_axis_value_flag_summary=source_stat_axis_value_flag_summary,
                generated_stat_axis_value_flag_summary=generated_stat_axis_value_flag_summary,
            )
        )
        stat_policy_override_suggestion, stat_policy_override_suggestion_reasons = (
            TtfInstancer._stat_policy_override_suggestion(stat_policy_recommendation)
        )
        return StatNamingDiagnostics(
            stat_policy=stat_policy,
            supported_stat_policies=STATIC_STAT_POLICIES,
            source_has_stat=source_has_stat,
            ...
            stat_policy_next_action=TtfInstancer._stat_policy_next_action(
                stat_policy_recommendation
            ),
            ...
            source_stat_axis_value_flags=source_stat_axis_value_flags,
            source_stat_axis_value_flag_meanings=source_stat_axis_value_flag_meanings,
            source_stat_axis_value_flag_summary=source_stat_axis_value_flag_summary,
            ...
            generated_stat_axis_value_flags=generated_stat_axis_value_flags,
            generated_stat_axis_value_flag_meanings=(
                generated_stat_axis_value_flag_meanings if generated_stat_axis_value_flags else ()
            ),
            generated_stat_axis_value_flag_summary=generated_stat_axis_value_flag_summary,
            notes=tuple(notes),
            warnings=tuple(warnings),
    ...
    @staticmethod
    def _stat_policy_recommendation(
        *,
        stat_policy: str,
        source_has_stat: bool,
        source_stat_name_ids: tuple[int, ...],
        ...
        if stat_policy == "static":
            reasons = ["static-policy-requested", "generated-static-stat-synthesized"]
            if generated_stat_axis_value_flag_summary.get("nonzero", 0):
                reasons.append("generated-axis-value-flags-nonzero")
            return "use-requested-static", tuple(reasons)
        if not source_has_stat:
            return "no-action", ("source-stat-absent",)

        reasons = ["source-stat-dropped-by-default"]
        if source_stat_axis_value_flag_summary.get("unknown", 0):
            reasons.append("source-axis-value-flags-unknown")
            return "review-before-drop", tuple(reasons)
        if uncovered_source_stat_name_ids:
            reasons.append("source-stat-name-ids-uncovered")
            return "review-before-drop", tuple(reasons)
        if source_stat_axis_value_flag_summary.get("nonzero", 0):
            reasons.append("source-axis-value-flags-nonzero")
            return "consider-static", tuple(reasons)
        if source_stat_name_ids:
            reasons.append("source-stat-name-ids-covered")
        else:
            reasons.append("source-stat-present-without-parseable-details")
        return "drop-with-diagnostics", tuple(reasons)

    @staticmethod
    def _stat_policy_override_suggestion(
        stat_policy_recommendation: str,
    ) -> tuple[str | None, tuple[str, ...]]:
        if stat_policy_recommendation == "consider-static":
            return "static", ("try-static-policy",)
        if stat_policy_recommendation == "drop-with-diagnostics":
            return "drop", ("drop-policy-currently-safe",)
        if stat_policy_recommendation == "review-before-drop":
            return None, ("manual-review-required",)
        if stat_policy_recommendation == "use-requested-static":
            return None, ("requested-static-policy-already-applied",)
        return None, ("no-override-needed",)

    @staticmethod
    def _stat_policy_next_action(stat_policy_recommendation: str) -> str:
        if stat_policy_recommendation == "no-action":
            return "none"
        if stat_policy_recommendation == "drop-with-diagnostics":
            return "keep-drop"
        if stat_policy_recommendation == "consider-static":
    ...
    @staticmethod
    def _static_stat_axis_value_flags(
        stat_policy: str,
        naming_strategy: str,
        legacy_style_label: str,
    ) -> int:
        ...
        if naming_strategy != "ribbi-safe":
            return 0
        if legacy_style_label in {"Bold", "Italic", "Bold Italic"}:
            return _STAT_LINKED_ELIDABLE_FLAGS
        return 0
